...
func (s *Server) DeleteAsset(ctx context.Context, request gen.DeleteAssetRequestObject) (gen.DeleteAssetResponseObject, error) {
	...
	if _, err := s.lifecycle.SoftDelete(ctx, request.AssetId); err != nil {
		return nil, newAPIError(http.StatusInternalServerError, "asset delete: internal error")
	}
...
func (s *Server) RestoreAsset(ctx context.Context, request gen.RestoreAssetRequestObject) (gen.RestoreAssetResponseObject, error) {
	...
	if errors.Is(err, lifecycle.ErrConflict) {
		return nil, newAPIError(http.StatusConflict, "asset is beyond the retention window and can no longer be restored")
	}
	if err != nil {
		return nil, newAPIError(http.StatusInternalServerError, "asset restore: internal error")
	}
	return gen.RestoreAsset200JSONResponse(toAsset(restored)), nil
}
...
func (s *Server) MergeAsset(ctx context.Context, request gen.MergeAssetRequestObject) (gen.MergeAssetResponseObject, error) {
	if request.Body == nil {
		return nil, newAPIError(http.StatusBadRequest, "missing request body")
	}
	survivorID := request.AssetId
	duplicateID := request.Body.DuplicateAssetId
	if survivorID == duplicateID {
		return nil, newAPIError(http.StatusBadRequest, "cannot merge an asset into itself")
	}
	...
	if errors.Is(err, lifecycle.ErrConflict) {
		return nil, newAPIError(http.StatusConflict, "conflicting state")
	}
	if err != nil {
		return nil, newAPIError(http.StatusInternalServerError, "asset merge: internal error")
	}

	dto := toAsset(survivor)
	...
	if merged, err := s.lifecycle.MergedAssets(ctx, survivorID); err == nil && len(merged) > 0 {
		...
		dto.MergedAssets = &list
	}

	return gen.MergeAsset200JSONResponse(dto), nil
}
...
func (s *Server) PatchAsset(ctx context.Context, request gen.PatchAssetRequestObject) (gen.PatchAssetResponseObject, error) {
	...
	if err != nil {
		return nil, newAPIError(http.StatusInternalServerError, "asset patch: internal error")
	}
	return gen.PatchAsset200JSONResponse(toAsset(updated)), nil
}
