...
func (s *Server) createImportBatch(w http.ResponseWriter, r *http.Request) {
	...
	src, err := s.factory.Sources.Get(r.Context(), batch.SourceID, repo.Tenant(tid))
	if err != nil {
...
// listImportBatches returns all of the requesting tenant's import batches
...
func (s *Server) listImportBatches(w http.ResponseWriter, r *http.Request) {
	...
	for _, b := range batches {
		src, err := s.factory.Sources.Get(r.Context(), b.SourceID, repo.Tenant(tid))
		if err != nil {
			writeImportError(w, err)
			return
		}
...
// source metadata. Unknown or foreign-tenant IDs yield 404.
