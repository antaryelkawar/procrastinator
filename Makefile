...
OAPI_CODEGEN_GENERATE := types,chi-server,strict-server
OPENAPI_TYPESCRIPT_VERSION := 7.13.0
REDOC_CLI_VERSION := 0.13.21
ORVAL_VERSION := 8.28.1
...
.PHONY: codegen codegen-go codegen-ts codegen-ts-client docs codegen-drift-check budget-check

## codegen: Run all code generators (Go server types, TS types, orval client, docs).
codegen: codegen-go codegen-ts codegen-ts-client docs
...
## codegen-ts-client: Generate the orval per-operation typed client from openapi.yaml.
## The pinned orval version + the committed ui/orval.config.ts make the output
## deterministic; the generated client (ui/src/lib/api/generated/orval/) is
## committed and drift-checked by codegen-drift-check.
codegen-ts-client:
	cd ui && npx orval@$(ORVAL_VERSION)
...
## (Go server gen, TS types, and the orval per-operation client.)
codegen-drift-check:
	...
	cd ui && npx vitest run src/lib/api/codegen.client.drift.test.ts

## budget-check: Assert codegen performance and size budgets (tasks 7.1/7.2/7.3).
## Covers the Go server gen, TS types, docs (Go budget tests) and the orval
## per-operation client budget (UI budget assertion).
budget-check:
	...
	cd ui && npx vitest run src/lib/api/codegen.client.drift.test.ts -t 'within the time budget'
