...
function makeUseAdd(outcomes: unknown[] | null) {
  ...
  const mutate = vi.fn(async (_body: unknown) => {
    if (outcomes !== null) {
      return outcomes;
    }
...
function renderPage(outcomes: unknown[] | null = null) {
  ...
  vi.mocked(addHooks.useAdd).mockReturnValue(addResult as never);
  vi.mocked(docsHooks.useAccounts).mockReturnValue({
    data: accounts,
    isLoading: false,
    error: null,
  } as never);
  const restoreMutate = vi.fn();
  vi.mocked(addHooks.useRestoreAsset).mockReturnValue({
