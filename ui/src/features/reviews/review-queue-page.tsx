...
import { useReviews, useApproveReview, useRejectReview } from '@/features/reviews/hooks';
...
function buildColumns(
  ...
  return [
    ...
    columnHelper.accessor('data.confidence', {
      header: 'Confidence',
    ...
    columnHelper.accessor('data.state', {
      header: 'Status',
      cell: (context) => (
        <Badge
          variant={
    ...
    }),
    {
      ...
      cell: (context) => {
        ...
        if (review.data.state !== 'pending') return null;
        return (
  ...
  return (
    <div className="space-y-6">
      {/* Shared top-right [+] chrome (task 13.4 / app-chrome) opens the
          directive-first review-ingest flow (14.5). */}
      <AddButton label="Add review" onAdd={() => setComposerOpen(true)} />
      ...
      <ReviewComposerFlow open={composerOpen} onOpenChange={setComposerOpen} />
