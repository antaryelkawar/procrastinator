...
import * as hooks from '@/features/reviews/hooks';
...
vi.mock('@/features/reviews/hooks', async () => {
  const actual = await vi.importActual('@/features/reviews/hooks');
  return {
    ...actual,
    useReviews: vi.fn(),
...
describe('ReviewQueuePage', () => {
  ...
  it('does not show approve/reject buttons for non-pending reviews (F-12)', () => {
    const approvedReview: IngestReview = {
      ...mockReviews[0],
      data: { ...mockReviews[0].data, state: 'approved', decided_at: '2024-01-16T10:00:00Z' },
    };
...
// The corner [+] control is present and wired to open the composer flow. The
// flow itself is closed by default (its Radix overlay renders nothing visible);
// it is exercised directly in review-composer-flow.test.tsx, so stub it here
// to prove the page mounts it without interfering with the queue assertions.
vi.mock('./review-composer-flow', async () => ({
  ReviewComposerFlow: ({ onOpenChange }: { onOpenChange: (open: boolean) => void }) => (
    <button type="button" onClick={() => onOpenChange(true)} aria-label="composer-flow-stub" />
  ),
}));

describe('ReviewQueuePage — add-review wiring', () => {
  it('renders the Add review corner [+] control (F-11)', () => {
    mockUseReviews.mockReturnValue({
      data: [],
      isLoading: false,
      isError: false,
    ...
    renderWithProviders();
    expect(screen.getByLabelText(/add review/i)).toBeInTheDocument();
  });
});
