  ...
  const processFileUpload = async (fileUpload: FileUpload) => {
    ...
    try {
      ...
      if (result.kind === 'committed') {
        ...
        // 'duplicate' — no review id; surface as held-without-id until the
        // dedicated duplicate affordance lands.
        setFiles((prev) => prev.map(f => f.id === fileUpload.id ? { ...f, state: 'held' } : f));
      }
