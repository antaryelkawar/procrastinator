...
vi.mock('@/features/finance/hooks', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/features/finance/hooks')>();
  return {
    ...actual,
    useMovements: vi.fn(),
...
beforeEach(() => {
  vi.mocked(docsHooks.useAccounts).mockReturnValue({ data: [], isLoading: false } as any);
...
describe('MovementsPage', () => {
  ...
  it('sends account_id/from/to to the client when filters are applied', async () => {
    vi.mocked(docsHooks.useAccounts).mockReturnValue({ data: [{ id: 'acc1', name: 'Main', type: 'bank', currency: 'INR', balance: '0' }], isLoading: false } as any);
