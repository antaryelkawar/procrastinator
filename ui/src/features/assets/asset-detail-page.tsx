...
import { useAssetDocuments } from '@/features/docs/hooks';
...
function buildDocumentColumns(): ColumnDef<TableFeatures, Document, unknown>[] {
  return [
    ...
    columnHelper.accessor('data.doc_type', {
      header: 'Type',
    }),
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold">
          {asset.data.brand ?? 'Asset'} {asset.data.model ?? ''}
        </h1>
        <Badge variant="outline">{asset.data.asset_category ?? 'other'}</Badge>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium">{asset.data.serial_number ?? '-'}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Purchase Date</p>
            <p className="font-medium">{asset.data.purchase_date ? formatDate(asset.data.purchase_date) : '-'}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Warranty Ends</p>
            <p className="font-medium">{asset.data.warranty_end ? formatDate(asset.data.warranty_end) : '-'}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Price</p>
            <p className="font-medium">
              {asset.data.price && asset.data.currency
                ? formatMoney(asset.data.price, asset.data.currency)
                : '-'}
            </p>
          </div>
        </CardContent>
