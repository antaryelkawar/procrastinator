...
import type { Asset, DuplicateReport, IngestReview } from '@/lib/api/generated/orval/procrastinator';
...
/**
 * AssetComposerFlow (task 14.4 / design D11, asset-registry spec) — the
 * directive-first asset creation flow wired to the /assets corner [+].
 *
 * The FIRST interaction is ONE describe-input — no brand/model/serial/purchase
 ...
 * Extraction then yields chips (ReviewChips) from the committed asset or the
 * held review; confirming the chips finalizes through the existing write path
 * (patchAsset for a committed asset, approveReview for a held review). A 409
 * duplicate outcome renders the shared DuplicatePromptDialog — the duplicate-serial
 * path is unchanged.
 *
 ...
 */
export function AssetComposerFlow({
  open,
  onOpenChange,
  onClosed,
}: {
  ...
  // Commit targets resolved at submit time (from the ingest outcome).
   ...
   * commit target (`assetId`/`reviewId`) are intentionally KEPT so the flow
   * advances to the review-chips step instead of falling back to the describe
   * surface. (A full reset is unnecessary — the flow is controlled and remounts
   * clean on each open.)
   */
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (attachments.length > 0) {
        ...
        // dropping the rest behind a false "Asset extracted" toast.)
        const doneIds = new Set<string>();
        let firstHeld: IngestReview | null = null;
        let firstCommitted: Asset | null = null;
        let heldCount = 0;
        ...
        for (const a of attachments) {
          try {
            ...
            if (result.kind === 'duplicate') {
              ...
              doneIds.add(a.id);
              committedCount += 1;
              if (firstCommitted === null) firstCommitted = result.asset;
            }
          } catch (e) {
        ...
        // without advancing to the chips step.
        if (duplicate !== null) {
          setDuplicateReport(duplicate);
          return;
        }

        // A clean batch: advance to the review-chips step for the FIRST held
        // (preferred) or committed result. The other files are already
        // committed/held server-side.
        if (failures.length === 0 && firstHeld !== null) {
          ...
          clearDescribe();
          toast.success(
            heldCount + committedCount === 1
              ? 'Asset extracted'
              : `${heldCount + committedCount} assets extracted`,
          );
          return;
        }
        if (failures.length === 0 && firstCommitted !== null) {
          setAssetId(firstCommitted.id);
          setReviewId(null);
          setFields(toReviewChips(firstCommitted.data));
          setOptionalFields([]);
          clearDescribe();
          toast.success(committedCount === 1 ? 'Asset extracted' : `${committedCount} assets extracted`);
          return;
        }

        // All failed (or nothing chips-able): stay on the describe step with
        // the error already surfaced above.
        return;
      }
      ...
      if (outcome.kind === 'asset_committed') {
        if (outcome.asset_id == null) {
          throw new Error('No asset id returned.');
        }
        const assetId = outcome.asset_id;
        setAssetId(assetId);
        setReviewId(null);
        const asset = await client.getAsset(user.activeUser ?? '', assetId);
        const chips = toReviewChips(asset.data);
        setFields(chips);
        setOptionalFields([]);
        toast.success('Asset extracted');
        clearDescribe();
        return;
      }
      // statement_preview / failed → friendly error, no chips.
      toast.error('Could not extract asset details. Try again.');
    } catch (e) {
      setError(friendlyMessage(e));
    } finally {
      setLoading(false);
    }
  }, [loading, describeText, note, attachments, ingestFile, ingestText, clearDescribe]);

  /**
   * Step 2 → finalize. Committed asset → patchAsset (only when the user
   * edited a chip); held review → approveReview (then patchAsset if the
   * confirmed values differ from the committed asset). No new endpoints.
   */
  const handleConfirm = useCallback(
    async (values: Readonly<Record<string, string>>) => {
      ...
      setError(null);
      try {
      // Baseline = the extracted values (before any user edit), keyed by
      // field.key. A confirmed value that differs from it is a user correction
      // and the ONLY thing carried to the write path: unedited values already
      // equal the committed/held data, so they need no write. This is what makes
      // the persisted payload exactly the confirmed chips.
      ...
      if (assetId !== null) {
        // Committed asset: patch only the user's corrections.
        if (Object.keys(body).length > 0) {
          await client.patchAsset(user.activeUser ?? '', assetId, body as never);
        }
      ...
      if (reviewId !== null) {
        // Held review: approve to commit (candidate_fields = the chips), then
        // patch any user corrections onto the committed asset.
    ...
    [assetId, reviewId, fields, user.activeUser, close],
  );
  ...
  const describeSurface = (
    <div className="w-full space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          id="describe-input"
          aria-label="Describe the asset"
          placeholder="Describe it — brand, model, anything you remember"
          value={describeText}
  ...
  const content = isDesktop ? (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{fields.length > 0 ? '' : 'Describe it, or attach a file.'}</DialogDescription>
        </DialogHeader>
        {body}
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[90vh] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Add an asset</SheetTitle>
          <SheetDescription>{fields.length > 0 ? '' : 'Describe it, or attach a file.'}</SheetDescription>
        </SheetHeader>
